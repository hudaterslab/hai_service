...
if [ "$DXRT_AUTO_PIP_INSTALL" = "true" ]; then
  if [ "$DXRT_FORCE_PIP_REINSTALL" != "true" ] && python -c "import dx_engine" >/dev/null 2>&1; then
    echo "[recorder-entrypoint] dx_engine already available"
    ...
    echo "[recorder-entrypoint] installing dx_engine from $BUILD_PKG_DIR"
    ...
    python -m pip install --no-cache-dir --break-system-packages "$BUILD_PKG_DIR" \
      ...
      || echo "[recorder-entrypoint] warning: dx_engine install failed"
  else
    echo "[recorder-entrypoint] warning: $DXRT_PY_PACKAGE_PATH not found, skip dx_engine install"
  fi
fi

exec python -m recorder.worker
