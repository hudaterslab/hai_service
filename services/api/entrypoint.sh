...
if [ "$DXRT_AUTO_PIP_INSTALL" = "true" ]; then
  if [ "$DXRT_FORCE_PIP_REINSTALL" != "true" ] && python -c "import dx_engine" >/dev/null 2>&1; then
    echo "[api-entrypoint] dx_engine already available"
    ...
    echo "[api-entrypoint] installing dx_engine from $BUILD_PKG_DIR"
    ...
    python -m pip install --no-cache-dir --break-system-packages "$BUILD_PKG_DIR" \
      ...
      || echo "[api-entrypoint] warning: dx_engine install failed"
  else
    echo "[api-entrypoint] warning: $DXRT_PY_PACKAGE_PATH not found, skip dx_engine install"
  fi
fi
